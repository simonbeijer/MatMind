...
export default function TextAreaField({ 
  ...
  return (
    <div className="flex flex-col">
      <label
        htmlFor={name}
        className="font-mono uppercase tracking-[0.18em] text-[11px] text-onboarding-text-muted mb-2"
      >
      ...
      <textarea
        ...
        className={`bg-onboarding-bg-primary border text-onboarding-text-primary placeholder:text-onboarding-text-subtle px-4 py-3 min-h-[80px] focus:outline-none w-full resize-none transition-colors ${
          displayError ? "border-cinnabar" : "border-onboarding-border-input focus:border-onboarding-text-primary"
        }`}
        {...props}
      />
      <div className="flex justify-between items-center mt-2">
        <div>
          {displayError && (
            <div
              id={`${name}-error`}
              className="font-mono uppercase tracking-[0.15em] text-[10px] text-cinnabar"
              role="alert"
            >
              {error || localError}
            </div>
        ...
        <div className="font-mono uppercase tracking-[0.15em] text-[10px] text-onboarding-text-subtle">
          {charCount}/{maxLength}
        </div>
      </div>
    </div>
